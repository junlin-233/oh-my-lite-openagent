...
describe("plugin safety", () => {
  ...
  it("ingests only first-level study courseware and reports generated outputs", async () => {
    ...
    try {
      ...
      const output = await hooks.tool?.bounded_lite_study_ingest?.execute({}, { directory: coursewareDir });
      ...
      expect(result.sources[0]).toMatchObject({ filename: "01-database.pdf", extension: ".pdf", status: "ok" });
  ...
  it("returns a recoverable blocker for legacy ppt when soffice is unavailable", async () => {
    ...
    try {
      ...
      expect(result.sources[0]).toMatchObject({ filename: "legacy.ppt", extension: ".ppt", status: "blocked" });
      expect(result.recoverableBlockers[0]).toMatchObject({ file: "legacy.ppt", recoverability: "recoverable", requiredTool: "soffice" });
    } finally {
  ...
  it("generates a bounded current-directory study package", async () => {
    ...
    try {
      ...
      const hooks = await createBoundedLitePlugin({ directory: coursewareDir });
      const output = await hooks.tool?.bounded_lite_study_package?.execute({}, { directory: coursewareDir });
      const result = JSON.parse(String(output));
  ...
  it("supports a source-only study package stage before full generation", async () => {
    ...
    try {
      ...
      const hooks = await createBoundedLitePlugin({ directory: coursewareDir });
      const output = await hooks.tool?.bounded_lite_study_package?.execute({ stage: "sources" }, { directory: coursewareDir });
      const result = JSON.parse(String(output));
  ...
  it("blocks study package generation for invalid AGENTS markers", async () => {
    ...
    try {
      ...
      expect(result.recoverableBlockers[0]).toMatchObject({ file: "AGENTS.md", recoverability: "recoverable" });
    } finally {
      await rm(coursewareDir, { recursive: true, force: true });
    }
  });
  ...
  it("returns warning metadata for mixed-state rebuild success", async () => {
    const configDir = await mkdtemp(path.join(os.tmpdir(), "omo-lite-planart-rebuild-warning-"));
    const previousConfigDir = process.env.OPENCODE_CONFIG_DIR;
    ...
    try {
      ...
      expect(rebuilt.firstSkippedInvalidFile).toBe("20260518-1030-a1b2c3d4/bad.md");
      expect(rebuilt.warnings?.[0]).toMatchObject({
        code: "PLANART_WARN_INVALID_PLAN_FILE_SKIPPED",
        path: "20260518-1030-a1b2c3d4/bad.md",
      });
    } finally {
      if (previousConfigDir === undefined) {
        delete process.env.OPENCODE_CONFIG_DIR;
  ...
  it("self-check mixed 场景不阻断 write，并把 warning 透传到工具结果", async () => {
    ...
    try {
      ...
      expect(writeAttempt.ok).toBe(true);
      expect(writeAttempt.warnings?.[0]).toMatchObject({
        code: "PLANART_WARN_INVALID_PLAN_FILE_SKIPPED",
        path: "20260518-1030-a1b2c3d4/bad.md",
      });
      expect(writeAttempt.skippedInvalidFileCount).toBe(1);
      expect(writeAttempt.firstSkippedInvalidFile).toBe("20260518-1030-a1b2c3d4/bad.md");
    } finally {
      if (previousConfigDir === undefined) {
        delete process.env.OPENCODE_CONFIG_DIR;
      } else {
  ...
  it("self-check 失败后修好文件，第二次调用会自动重试并恢复", async () => {
    ...
    try {
      ...
      await writeFile(badPlanPath, [
        ...
        "plan_id: c1d2e3f4",
        "title: fixed",
        "session_key: 20260518-1030-a1b2c3d4",
      ...
      expect(second.ok).toBe(true);
      expect(second.path).toMatch(/after-2\.md$/);
    } finally {
      if (previousConfigDir === undefined) {
        delete process.env.OPENCODE_CONFIG_DIR;
      } else {
  ...
  it("self-check 失败后修复完成时，并发调用只触发一次 retry self-check", async () => {
    const configDir = await mkdtemp(path.join(os.tmpdir(), "omo-lite-planart-selfcheck-concurrent-retry-"));
    const previousConfigDir = process.env.OPENCODE_CONFIG_DIR;
    ...
    try {
      ...
      expect(first.ok).toBe(false);
      expect(selfCheckAttempts).toBe(1);
      ...
      const [second, third]: [{ ok: boolean }, { ok: boolean }] = await Promise.all([
        execPlanArtifact(
          hooks,
          {
            action: "write",
      ...
      expect(selfCheckAttempts).toBe(2);
    } finally {
      if (previousConfigDir === undefined) {
        delete process.env.OPENCODE_CONFIG_DIR;
      } else {
