...
- Return the plan as a chat artifact plus a `recommended_plan_path` under `.liteagent/plans/`. Write the final plan artifact yourself only after explicit user approval to save/write/persist the plan.
- You may create, update, and maintain plan artifacts under `.liteagent/plans/` and the local index `.liteagent/plan-index.jsonl` after that approval.
